...
import {
  ...
  visionLineForHumans,
} from '@/modules/chat/helpers/photo-vision-stored';
...
import { buildPhotoRecallMetadata } from '@/modules/chat/helpers/build-photo-recall-metadata';
import { buildPhotoRecallPromptSection } from '@/modules/chat/helpers/build-photo-recall-prompt-section';
import { orderedMessageIdsFromRecallItems } from '@/modules/chat/helpers/resolve-photo-recall-message-ids';
...
import { linkPreviewsSerializedFromRows } from '@/modules/chat/helpers/chat-link-client-preview.helpers';
import {
...
} from '@/modules/chat/helpers/chat-question-temporal';
import { shouldEscalateTrivialForPhotoFollowup } from '@/modules/chat/helpers/photo-offer-followup';
...
import {
  ...
  parseIsoDate,
} from '@/modules/chat/helpers/chat-orchestrator.helpers';
...
export class ChatOrchestratorService {
  ...
  private async mergeSummary(
    ...
    try {
      ...
      if (result && result.length > 2000) {
        try {
          result =
            (await this.orchestratorLlm.compressProfileSummary(result)).trim() ||
            result;
        } catch {
  ...
  private async seedSummaryIfReady(userId: string): Promise<void> {
    ...
    const text = await this.orchestratorLlm.seedUserSummaryFromMessages(
      msgTexts,
    );
