...
import { LINK_CACHE_TTL_MS } from '@/modules/chat/constants/link-reading.constants';
import type { JinaSnapshotStored } from '@/modules/chat/types/jina-snapshot-stored.types';
import { linkTitleHintFromMarkdown } from '@/core/utils/link-title-from-markdown';
import { normalizeJinaSnapshotForStorage } from '@/modules/chat/utils/normalize-jina-snapshot-for-storage';
import type { JinaReaderService } from '@/modules/chat/jobs/jina-reader.service';
import type { LinkContentClassifierService } from '@/modules/chat/jobs/link-content-classifier.service';
import { isLikelySocialRestrictedHost } from '@/modules/chat/jobs/restricted-link-hosts';
import { looksLikeLoginWallMarkdown } from '@/modules/chat/jobs/link-login-wall-heuristic';
...
export async function processMessageLinkOccurrence(
  ...
  const jinaResult = await jina.fetchPage(normalized, { userId, messageId });
