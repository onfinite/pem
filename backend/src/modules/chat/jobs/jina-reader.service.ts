...
import { LINK_JINA_FETCH_TIMEOUT_MS } from '@/modules/chat/constants/link-reading.constants';
import type { JinaSnapshotStored } from '@/modules/chat/types/jina-snapshot-stored.types';
import { parseJinaReaderJsonBody } from '@/modules/chat/utils/parse-jina-reader-json';
...
export type JinaReaderLogContext = {
  userId?: string;
  messageId?: string;
};
...
export class JinaReaderService {
  ...
  async fetchPage(
    targetHref: string,
    logCtx?: JinaReaderLogContext,
  ): Promise<JinaReaderResult> {
    const lc = logCtx ?? {};
    const apiKey = this.config.get<string>('jina.apiKey');
    if (!apiKey) {
      this.log.warn(
        logWithContext('JINA_API_KEY not set — link reading disabled', {
          scope: 'jina_reader',
          ...lc,
        }),
      );
    ...
    try {
      ...
      if (!res.ok) {
        this.log.warn(
          logWithContext(`Jina non-OK status=${res.status}`, {
            scope: 'jina_reader',
      ...
      if (!parsed) {
        this.log.warn(
          logWithContext('Jina JSON parse failed', {
            scope: 'jina_reader',
            httpStatus: res.status,
            ...lc,
          }),
      ...
      this.log.warn(
        logWithContext(`Jina fetch ${isAbort ? 'timeout' : 'error'}`, {
          scope: 'jina_reader',
          timedOut: isAbort,
          ...lc,
        }),
