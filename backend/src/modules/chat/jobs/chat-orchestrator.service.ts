...
import {
  compressProfileSummary,
  extractLightweightMemoryJson,
  generateOffTopicRedirectText,
  mergeUserSummaryWithNewInfo,
  seedUserSummaryFromMessages,
} from '@/modules/chat/agents/orchestrator-llm';
...
import { reconcileRelativePeriodLabel } from '@/modules/extracts/reconcile-relative-period-label';
import {
  decodePhotoVisionStored,
  visionLineForHumans,
} from '@/modules/chat/utils/photo-vision-stored';
...
import { buildPhotoRecallMetadata } from '@/modules/chat/jobs/build-photo-recall-metadata';
import { buildPhotoRecallPromptSection } from '@/modules/chat/jobs/build-photo-recall-prompt-section';
import { orderedMessageIdsFromRecallItems } from '@/modules/chat/jobs/resolve-photo-recall-message-ids';
...
import { linkPreviewsSerializedFromRows } from '@/modules/chat/jobs/link-previews-for-client';
import {
...
} from '@/modules/chat/jobs/chat-question-temporal';
import { shouldEscalateTrivialForPhotoFollowup } from '@/modules/chat/jobs/photo-offer-followup';
import { normalizeDedupeTaskKey } from '@/core/utils/filter-deduped-creates';
import { clampAgentOutput } from '@/modules/chat/jobs/chat-orchestrator-clamp-output';
import {
  buildRrule,
  parseIsoDate,
} from '@/modules/chat/jobs/chat-orchestrator-rrule';
import {
  displayUrlsFromMessageLinkRows,
  mergeMessageLinksIntoExtractPemNote,
} from '@/modules/chat/utils/merge-message-links-into-extract-pem-note';
...
export class ChatOrchestratorService {
  ...
  private tryLightweightMemoryExtraction(
    ...
    void (async () => {
      try {
        ...
        const text = await extractLightweightMemoryJson({
          apiKey,
          contentSlice: content.slice(0, 500),
        });
  ...
  private async generateOffTopicRedirect(userMessage: string): Promise<string> {
    ...
    try {
      ...
      const text = await generateOffTopicRedirectText({
        apiKey,
        userMessageSlice: userMessage.slice(0, 500),
      });
  ...
  private async mergeSummary(
    ...
    try {
      ...
      let result =
        (await mergeUserSummaryWithNewInfo({
          apiKey,
          existing,
          newInfo: trimmed,
        })) || existing;
      ...
      if (result && result.length > 2000) {
        try {
          result =
            (await compressProfileSummary({ apiKey, text: result })).trim() ||
            result;
        } catch {
  ...
  private async seedSummaryIfReady(userId: string): Promise<void> {
    ...
    const text = await seedUserSummaryFromMessages({
      apiKey,
      msgTexts,
    });
