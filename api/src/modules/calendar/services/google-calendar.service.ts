...
} from '@/modules/calendar/helpers/sign-google-oauth-state';
...
export class GoogleCalendarService {
  ...
  async exchangeCode(code: string): Promise<{
    ...
    try {
      ...
      this.log.warn(
        logWithContext('Could not fetch Google user email', {
          scope: 'google_calendar',
          phase: 'oauth_userinfo',
        }),
      );
    }
  ...
  async fetchEvents(
    ...
    logContext: { userId: string; connectionId: string },
  ): Promise<{
    events: GoogleEvent[];
    ...
    for (const calId of calendarIds) {
      ...
      try {
        ...
        if (code === 410 && isPrimary) {
          this.log.log(
            logWithContext('Sync token expired, full sync for primary', {
              ...logContext,
              calendarId: 'primary',
              scope: 'google_calendar_fetch',
            }),
          );
          ...
          this.log.warn(
            logWithContext('Failed to fetch Google Calendar events', {
